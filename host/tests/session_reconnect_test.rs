...
async fn test_session_reconnection() {
    ...
    let session_id = {
        ...
        let msg = stream.next().await.expect("No message").expect("Error reading");
    ...
    // Step 2: Reconnect with same Session ID
    {
        ...
        let (mut stream, _) = connect_async(&reconnect_url).await.expect("Failed to reconnect");
    ...
    // Step 3: Connect with NEW session explicitly
    {
        ...
        let (mut stream, _) = connect_async(&new_url).await.expect("Failed to connect new");
